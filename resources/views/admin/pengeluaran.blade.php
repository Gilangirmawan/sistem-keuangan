            ...
            <tbody>
                ...
                <tr class="odd:bg-white even:bg-gray-50 border-b">
                    ...
                    <td class="px-6 py-4 flex gap-2">
                        <button onclick="openEditModal('{{ route('admin.pengeluaran.update', $item->id) }}', '{{ $item->keterangan }}', '{{ $item->id_kategori }}', '{{ $item->jumlah }}', '{{ $item->total }}')" 
                                class="bg-yellow-400 hover:bg-yellow-500 text-white px-2 py-1 rounded"><i class="fa-solid fa-pencil"></i> Edit</button>
                        <button onclick="openDeleteModal('{{ route('admin.pengeluaran.destroy', $item->id) }}')" 
