<aside 
    class="fixed inset-y-0 left-0 z-30 w-64 bg-gray-900 text-white flex flex-col transform transition-transform duration-300 ease-in-out md:translate-x-0"
    ...
    <nav class="flex-1 p-2 space-y-1 overflow-y-auto">
        <a href="{{ route('admin.dashboard') }}"
            class="flex items-center gap-3 px-4 py-2 rounded {{ request()->routeIs('admin.dashboard') ? 'bg-sky-500 text-white' : 'hover:bg-gray-800' }}">
        ...
        <div x-data="{ open: {{ request()->routeIs(['admin.pemasukan.*', 'admin.pengeluaran.*']) ? 'true' : 'false' }} }">
            ...
            <div x-show="open" x-transition class="mt-1 ml-5 pl-3 border-l-2 border-gray-700 flex flex-col">
                <a href="{{ route('admin.pemasukan.index') }}"
                    class="flex items-center gap-3 px-4 py-2 rounded text-sm {{ request()->routeIs('admin.pemasukan.*') ? 'bg-sky-500 text-white' : 'hover:bg-gray-800' }}">
                    <i class="fa-solid fa-wallet w-5 text-center"></i> <span>Pemasukan</span>
                </a>
                
                <a href="{{ route('admin.pengeluaran.index') }}" 
                    class="flex items-center gap-3 px-4 py-2 rounded text-sm {{ request()->routeIs('admin.pengeluaran.*') ? 'bg-sky-500 text-white' : 'hover:bg-gray-800' }}">
        ...
        <div x-data="{ open: {{ request()->routeIs(['admin.laba.*', 'admin.buku.*', 'admin.kas.*']) ? 'true' : 'false' }} }">
            ...
            <div x-show="open" x-transition class="mt-1 ml-5 pl-3 border-l-2 border-gray-700 flex flex-col">
                <a href="{{ route('admin.laba.index') }}"
                    class="flex items-center gap-3 px-4 py-2 rounded text-sm {{ request()->routeIs('admin.laba.*') ? 'bg-sky-500 text-white' : '' }}">
                    <i class="fa-solid fa-chart-line w-5 text-center"></i> <span>Laba Dan Rugi</span>
                </a>
                
                <a href="{{ route('admin.buku.index')}}" 
                    class="flex items-center gap-3 px-4 py-2 rounded text-sm {{ request()->routeIs('admin.buku.*') ? 'bg-sky-500 text-white' : '' }}">
                    <i class="fa-solid fa-book w-5 text-center"></i> <span>Buku Besar</span>
                </a>

                <a href="{{ route('admin.kas.index')}}" 
                    class="flex items-center gap-3 px-4 py-2 rounded text-sm {{ request()->routeIs('admin.kas.*') ? 'bg-sky-500 text-white' : '' }}">
